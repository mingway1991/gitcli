...
def delete_branch(path, branch):
    ...
    if errCode == 0:
        ...
        click.secho('删除分支失败:%s' % (errMsg), fg='red')
        sys.exit(1)
...
def clean_branches(path):
    ...
    if errCode == 0:
        ...
        for local_branch in local_branches:
            ...
            if local_branch_name != 'master' \
                ...
                if errCode == 0:
                    ...
                    click.secho('删除分支%s失败:%s' % (local_branch_name, errMsg), fg='red')
                    sys.exit(1)
        ...
        if errCode == 0:
            ...
            click.secho('列出分支失败:%s' % (errMsg), fg='red')
            sys.exit(1)
    else:
        click.secho('列出分支失败:%s' % (errMsg), fg='red')
        sys.exit(1)
...
def merge(path, branch):
    ...
    if len(conflict_files) != 0:
        ...
        if len(cannot_fix_conflict_files) > 0:
            click.secho('不能解决冲突文件列表:%s' % (cannot_fix_conflict_files), fg='red')
            sys.exit(1)
    all_modify_files = []
    # 列出没有暂存的修改文件
